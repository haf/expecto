...
module Dummy =
    ...
    let thisModuleType = lazy Type.GetType "Expecto.Dummy, Expecto.Tests"

module EmptyModule =
    let thisModuleType = lazy Type.GetType "Expecto.EmptyModule, Expecto.Tests"
...
module Tests =
    ...
    let tests =
        TestList [
            testCase "basic" <| fun _ -> Expect.Equal("2+2", 4, 2+2)
            ...
            testList "transformations" [
                testCase "multiple cultures" <| fun _ ->
                    ...
                    Expect.Equal("results count", 3, results.Count)
                    let inline assertTrue msg x = Expect.Equal(msg, true, x)
            ...
            testList "assertions" [
                testList "NotEqual" [
                    testCase "pass" <| fun _ ->
                        Expect.NotEqual("should be different", "", "monkey")
                ...
                testList "raise" [
                    testCase "pass" <| fun _ ->
                        Expect.Raise("", typeof<ArgumentNullException>, fun _ -> nullArg "")

                    testCase "fail with incorrect exception" <| fun _ ->
                        let test () = Expect.Raise("", typeof<ArgumentException>, fun _ -> nullArg "")
                        assertTestFails test

                    testCase "fail with no exception" <| fun _ ->
                        let test () = Expect.Raise("", typeof<ArgumentNullException>, ignore)
                        assertTestFails test
                ]

                testList "string contain" [
                    testCase "pass" <| fun _ ->
                        Expect.StringContains("", "hello", "hello world")

                    testCase "fail" <| fun _ ->
                        let test () = Expect.StringContains("", "a", "hello world")
                        assertTestFails test
                ]
            ]
            ...
            testList "computation expression" [
                ...
                for c in [-5; 1; 6] ->
                    testCase (sprintf "compare comp.exp. and normal with value %d" c) <| fun _ ->
                        ...
                        Expect.Equal("result", normalTag, compexpTag)
            ]
        ]
